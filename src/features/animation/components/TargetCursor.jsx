...
import CalloutWithLeader from './CalloutWithLeader';
...
const TargetCursor = memo(function TargetCursor({
  ...
  const cursorQuickX = useRef(null);
  const cursorQuickY = useRef(null);
  ...
  const moveCursor = useCallback((x, y) => {
    ...
    if (cursorQuickX.current && cursorQuickY.current) {
  ...
  useEffect(() => {
    ...
    cursorQuickX.current = gsap.quickTo(cursor, 'x', { duration: 0.12, ease: 'power3.out' });
    cursorQuickY.current = gsap.quickTo(cursor, 'y', { duration: 0.12, ease: 'power3.out' });
